...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    if (ul_reason_for_call == DLL_PROCESS_ATTACH)
        printf("[StubModule] DllMain DLL_PROCESS_ATTACH\n");
    else if (ul_reason_for_call == DLL_PROCESS_DETACH)
        printf("[StubModule] DllMain DLL_PROCESS_DETACH\n");
    return TRUE;
}

class StubUiModule final : public IUiModule
{
public:
    StubUiModule() = default;
    ~StubUiModule() override = default;

    void onLoad(UiHostServices& svc, domain::DataContext& dataContext) override
    {
        printf("[StubModule] onLoad\n");
        m_app = reinterpret_cast<core::Application*>(svc.app);
        m_config = reinterpret_cast<ports::IConfigPort*>(svc.config);

        if (m_app)
            m_app->setRibbonbarCallback([]() {});

        registerSettings();
        printf("[StubModule] onLoad done\n");
    }

    void onUnload(UiHostServices& svc, domain::DataContext& dataContext) override
    {
        printf("[StubModule] onUnload\n");
...
private:
    ...
    void registerSettings()
    {
        ...
        m_config->registerSetting(SettingInfo(
            "pistachio.UI", "config.showAdvanced", "Show advanced settings - changed",
...
IUiModule* pistachio_create_ui_module() { 
    printf("[StubModule] pistachio_create_ui_module called\n");
    return new StubUiModule();
}
...
static const UiPluginManifestV1 g_manifest = {
    ...
    "pistachio.stub", "Pistachio UI Stub", "0.1.1", "UI"
};
